...
public class PersonalDAO extends conexion.ConexionOracle implements Serializable {
    ...
    public List<Personal> traeRegistrosPersonalBusqueda(int valor, int noUsuario, String busqueda) {
        List<Personal> listaPersonal;
        System.out.println("Entro a cargar la lista de registros personal busqueda");
        Personal per = null;
        ResultSet rs = null;
        String query = null;
        String continuacion = valor==3 ? "AND EXISTS(SELECT null\n" +
                                "FROM MOBILIARIO MO,\n" +
                                "  USUARIOS_DEPARTAMENTO UD\n" +
                                "where MO.NOPERSONALBM = PE.NOPERSONALBM\n" +
                                "AND   MO.IDDEPARTAMENTO = UD.IDDEPARTAMENTO\n" +
                                "AND   MO.IDESTATUS = 3 \n" +
                                "AND UD.NO_USUARIO     = "+noUsuario+")" : "";
        query = "select PE.nopersonalbm, "
                + "PE.nopersonal, "
                + "PE.rfc, "
                + "PE.curp, "
                + "PE.ap1, "
                ...
                + "PE.funcion "
                + "from personal PE "
                + " where (PE.nombre||' '||PE.ap1||' '||PE.ap2 like '%" + busqueda + "%' OR PE.rfc like '%" + busqueda + "%') "+continuacion;
        System.out.println(query);
        try {
            listaPersonal = new ArrayList<>();
            super.abrirConexion();
