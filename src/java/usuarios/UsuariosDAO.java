...
public class UsuariosDAO extends conexion.ConexionOracle implements Serializable {
    ...
    public int insertaUsuario(Usuarios us) {
        ...
        final String query = "INSERT INTO USUARIOS (NO_USUARIO, CLAVE, PASAPORTE, NOMBRE,APELLIDO_PATERNO,APELLIDO_MATERNO, "
                + "   CORREO, TELEFONO, IDESTATUS, FECHA_AUDITORIA, NOPERSONALBM) "
    ...
    public int actualizaUsuario(Usuarios us) {
        ...
        final String query = "UPDATE USUARIOS set CLAVE = ?, NOMBRE = ? ,"
                + " APELLIDO_PATERNO = ? ,APELLIDO_MATERNO = ? ,CORREO = ? , TELEFONO = ?, NOPERSONALBM = ? "
                + " where NO_USUARIO = ? ";
    ...
    public void eliminaDepUsuario(Usuarios us) {
        ...
        System.out.println("Entro a eliminar valor: " + us.getNoUsuario());
        ResultSet rs = null;
        final String query = "DELETE FROM ENLACE WHERE NOUSUARIO = ? ";
        try {
            super.abrirConexion();
            rs = super.getPreparedStatement(query, new Object[]{us.getNoUsuario()}).executeQuery();
        ...
        try {
            ...
            for (Departamento x : deps) {
                if (x.isDepartamentoSeleccionado()) {
                    esenclace = x.isDepartamentoSeleccionadoEnlace() ? "S" : "N";
                    query = "INSERT INTO ENLACE (IDDEPARTAMENTO, NOUSUARIO, ESENLACE, IDESTATUS) VALUES (?,?,?,1) ";
                    super.getPreparedStatement(query, new Object[]{x.getIdDepartamento(), usuario, esenclace}).executeQuery();
                    query = "";
                    valor++;
                }
            }
    ...
    public List<Departamento> traeRegistrosdep(Usuarios us) {
        ...
        query = "SELECT dep.iddepartamento,\n"
                + "  dep.nomdepto,\n"
                + "  dep.idur,\n"
                + "  dep.idestatus, (SELECT EN.NOUSUARIO FROM ENLACE EN WHERE EN.IDDEPARTAMENTO = DEP.IDDEPARTAMENTO AND EN.NOUSUARIO= ? ) DEPUSUARIO,"
                + " (SELECT EN.ESENLACE FROM ENLACE EN WHERE EN.IDDEPARTAMENTO = DEP.IDDEPARTAMENTO AND EN.ESENLACE = 'S' AND EN.NOUSUARIO= ? ) ESENLACEUSUARIO \n"
        ...
        try {
            ...
            super.abrirConexion();
            rs = super.getPreparedStatement(query, new Object[]{
                us.getNoUsuario(), us.getNoUsuario()}).executeQuery();
            ...
            while (rs.next()) {
                ...
                boolean esenlace;
                if(rs.getString("ESENLACEUSUARIO")!=null){
                esenlace = rs.getString("ESENLACEUSUARIO").equals("S");}
                else{esenlace=false;}
                d.setDepartamentoSeleccionadoEnlace(esenlace);
